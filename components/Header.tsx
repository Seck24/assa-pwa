...
export default function Header({ title, showBack = true, backHref }: HeaderProps) {
  ...
  return (
    <header className="flex items-center justify-between px-4 py-3 bg-assa-green sticky top-0 z-40">
      <button
        onClick={handleBack}
        className={`text-white text-2xl font-bold w-8 ${!showBack ? 'invisible' : ''}`}
      >
        ←
      </button>
